...
$message = '<div class="notif success bloc">
    signature je uspješno izmenjena. 
    <a href="#" class="close"></a>
</div>';
...
$subject = "Uniq Hosting - Game Panel Informacije";
$header = "From: $from";
$message = 'Postovani '.$fname.' '.$lname.',

Vas server je uspjesno kreiran

Login podaci za Game Panel su sljedeci:
Email: '.$to.'
Lozinka: '.$password.'
Game Panel Link: '.$link.'

Kako dodati admina i sve ostale informacije neophodne za upravljanje vasim serverom mozete nas kontaktirati putem Tiketa 
...
Nakon isteka servera,server ostaje aktivan jos 5 dana.Ukoliko ni tada nebude placen server se automacki suspenduje.
...
Za sva pitanja kontaktirajte nas na email: hostinguniq@gmail.com


Lep pozdrav!

Uniq Hosting
www.uniq-hosting.info
';
...
$to = $email;	
$from = $admin_info["hostinguniq@gmail.com"];
$subject = "Uniq Hosting - Novi password";
$header = "From: $from";
$message = 'Postovani '.$fname.' '.$lname.'
...
Ukoliko imate bilo kakvih pitanja ili problema mozete nas kontaktirati na hostinguniq@gmail.com
...
if($_POST["dodaj_radnika"]){
   ...
   if($_POST["email"]){
       ...
       if($username==0){
          ...
          $status = $_POST["status"];
		  if($status=="support"){
			$update_profile = mysql_query('INSERT INTO admin (fname, lname, username, password, email, status, napomena, boja) VALUES ("'.$ime.'", "'.$prezime.'", "'.$username.'", "'.$password.'", "'.$email.'", "'.$status.'", "Dobrodosao na e-trail hosting! :)", "#0A93CC")');
		  }else{
			$update_profile = mysql_query('INSERT INTO admin (fname, lname, username, password, email, status, napomena, boja) VALUES ("'.$ime.'", "'.$prezime.'", "'.$username.'", "'.$password.'", "'.$email.'", "'.$status.'", "Dobrodosao na e-trail hosting! :)", "red")');		  
		  }
...
if($_POST["mass_email"]){
    ...
    if($_POST["korisnici"]){
        ...
        if($_POST["naslov"]){
			...
			$headers .= 'From: uniq-hosting.info Uniq-Hosting' . "\r\n";
