...
if($sampp2['mod']=="7"){
	...
	$graph = "<img style='margin-left:10px;width:250px;height:120px;' src='http://arenahosting.info/chart/$server_info[ip]:$server_info[port]'>";
...
$smarty->assign('error', "Netacni username i lozinka"); 		
}
...
$fajov = $_GET["file"];
$folder = 'cache/other_c/'.$server_info["username"].'_'.$fajov.'';
...
$contents = fread($handle, filesize($filename)) or $smarty->assign('message', "Ne mogu procitati fajl");
...
if($_GET["page"] == "plugini") {
	...
	if(empty($id)) {
		die("<script> alert('Odabrani server, ne pripada vama !'); document.location.href='?page=serveri'; </script>");
	}
	...
	$plugins = array();
	if(!$connect) {
		die("<script> alert('Nemogu da se povezem sa FTP Serverom !'); document.location.href='?page=serveri'; </script>");
	} else if(!$login) {
		die("<script> alert('Nemogu da se povezem sa FTP Serverom !'); document.location.href='?page=serveri'; </script>");
	}
	...
	if($task == "install") {
		...
		$ime_plugina = "mdb_ban_system.amxx";
		...
		//$folder = 'cache/other_c/ftp/ftp_arena_gp_'.$ftpuser.'_users.ini';
		//$fw = @fopen(''.$folder.'', 'r');
...
$smarty->display('plugin_lista.tpl');
}
...
if($_POST["fname"]){
	...
	if($client["email"]==$configs["email"]){
		$smarty->assign('message', "Hocel to majstore :)");	
	} else {
